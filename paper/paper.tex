...
The completed $2\times2$ matrix suggests a natural design target for future work: architectures that can selectively regulate gradient flow per timescale, capturing flat architectures' ability to exploit good gradients \emph{and} hierarchical architectures' robustness to poor gradients. Whether this is achieved through gradient gating, architectural hybridization, adaptive unrolling, or an entirely different mechanism remains an open question---but the interaction effect documented here establishes it as a concrete, measurable goal.
